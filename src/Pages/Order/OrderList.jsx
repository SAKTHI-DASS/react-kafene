...
const OrderList = () => {
  ...
  const getInput = (e) => {
    ...
    setFilters((prevFilter) => {
      ...
      const lengthcount = orderList.filter(
        (order) => nextFilter[order.orderStatus] !== false,
      );
        ...
        <div className="main-box">
          <div className="filter-part">
            ...
            <div className="filter-list">
              ...
              <label className="ckeck-box-list">
                <input
                  type="checkbox"
                  name="New"
                  onChange={getInput}
          ...
          <div style={{ width: "100%" }}>
            <table className="orders">
              ...
              <tbody id="table-body">
                {orderList &&
                  orderList
                    .filter((order) => filters[order.orderStatus] !== false)
                    .map((data) => {
                      return (
                        <tr class="orderlist" key={data.id}>
                          <td class="light-font">{data.id}</td>
                          <td class="dark-font">{data.customerName}</td>
                          <td class="dark-font">
                            {data.orderDate}
                            <br />
                            <span class="light-font">{data.orderTime}</span>
                          </td>
